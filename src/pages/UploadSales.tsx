...
import { saveSalesImportRecord } from '../utils/salesImportStorage';
...
export const UploadSales = () => {
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        saveSalesImportRecord({
          id: `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
          fileName: file.name,
          importedBy: role ? role.toUpperCase() : 'UNKNOWN',
          importedRole: role || 'unknown',
          importedAt: new Date().toISOString(),
          totalRows: transformedData.length,
          totalAmount: transformedData.reduce((sum, row) => sum + row.amount, 0),
        });
  ...
  const downloadTemplate = () => {
    // Create sample data
    const sampleData = [
      { date: '2024-01-15', billNo: 'INV-001', product: 'Industrial Solvent A', salesman: 'John Doe', quantity: 50, price: 45.50, amount: 2275.00 },
      { date: '2024-01-15', billNo: 'INV-002', product: 'Polymer Mix B', salesman: 'Sarah Smith', quantity: 30, price: 125.00, amount: 3750.00 },
      { date: '2024-01-16', billNo: 'INV-003', product: 'Adhesive X-200', salesman: 'Mike Johnson', quantity: 100, price: 85.00, amount: 8500.00 },
      { date: '2024-01-16', billNo: 'INV-004', product: 'Industrial Solvent A', salesman: 'John Doe', quantity: 75, price: 45.50, amount: 3412.50 },
      { date: '2024-01-17', billNo: 'INV-005', product: 'Coating Z-10', salesman: 'Emily Davis', quantity: 20, price: 210.00, amount: 4200.00 },
    ];
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-stone-900">Upload & Analyze Sales</h1>
      ...
      {salesData.length > 0 && (
        <div ref={dashboardRef} className="space-y-6 bg-stone-50 p-6 rounded-xl">
          ...
          <div className="flex justify-center gap-4">
            ...
            <button
              onClick={downloadTemplate}
              className="flex items-center gap-2 px-6 py-3 bg-blue-50 text-blue-700 border-2 border-blue-200 rounded-lg hover:bg-blue-100 transition-colors font-medium shadow-sm"
            >
              <Download className="w-4 h-4" />
              Download Template
            </button>
